package lang.thegodofjava.Chapter07;

public class ManagerHeight {

    public static void main(String[] args) {
        ManagerHeight managerHeight = new ManagerHeight();
        managerHeight.setData();

        for (int i = 0; i < 5; i++) {
//            managerHeight.printHeight(i);
            managerHeight.printAverage(i);
        }
    }

    int[][] gradeHeights = new int[5][];

    public void setData() {
        gradeHeights[0] = new int[]{170, 180, 173, 175, 177};
        gradeHeights[1] = new int[]{160, 165, 167, 186};
        gradeHeights[2] = new int[]{158, 177, 187, 176};
        gradeHeights[3] = new int[]{173, 182, 181};
        gradeHeights[4] = new int[]{170, 180, 165, 177, 172};
    }

    public void printHeight(int classNo) {
        int classNumber = classNo + 1;
        System.out.println("classNo.: " + classNumber);

        for (int i = 0; i < gradeHeights[classNo].length; i++) {
            System.out.println(gradeHeights[classNo][i]);
        }
    }

    public void printAverage(int classNo) {
        int classNumber = classNo + 1;
        System.out.println("classNo.: " + classNumber);

        double sum = 0;
        int studentCount = gradeHeights[classNo].length;

        for (int i = 0; i < studentCount; i++) {
            sum += gradeHeights[classNo][i];
        }
        System.out.println("Height sum = " + (sum / studentCount));
    }

/*    public void printHeight(int classNo) {
        System.out.println("ClassNo." + classNo);
        for (int data : gradeHeights[classNo - 1]) {
            System.out.println(data);
        }
    }
    public void printAverage(int classNo) {
        System.out.println("ClassNo." + classNo);
        double sum=0;
        int studentCount=gradeHeights[classNo - 1].length;
        for (int data : gradeHeights[classNo - 1]) {
            sum+=data;
        }
        System.out.println(sum/studentCount);
    }*/
}
